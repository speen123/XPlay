...
bool FFDemux::Seek(double pos)
{
    ...
    avformat_flush(ic);
    long long seekPts = 0;
    seekPts = (long long)(ic->streams[videoStream]->duration * pos);

    //往后跳转到关键帧                        跳转到关键帧 即往时间前面跳转
    if(av_seek_frame(ic,videoStream,seekPts,AVSEEK_FLAG_FRAME|AVSEEK_FLAG_BACKWARD) >= 0){
        re = true;
    }
    else{
        re = false;
